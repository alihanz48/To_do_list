...
import 'dart:developer' as msgman;
...
class _todoListState extends State<todoList> {
  ...
  basla() async {
    ...
    if (compId == null.toString()) {
      uyari();
    }
    await vericek(compId);
    await getGroups(compId);
  }

  Future<void> vericek(String compId) async {
    await datad.allWorks(compId);
    setState(() {
      mainworks = datad.Works; // List değişikliğini bildir
    });
  }
  ...
  void conf(int index, bool? value, String person) async {
    await datad.worksConf(mainworks[index]['id'].toString(), value, person);
    await vericek(compId);
  }
        ...
        return const AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Text(
                "Henüz herhangi bir işletme sizi iş ağına dahil etmedi",
  ...
  Future<void> add_task(
    ...
    await datad.addWork(name.text, iletisim.text, aciklama.text, adres.text,
        gId == "" ? "1" : gId, compId, box.get("user").toString());
  }
  ...
  Future<void> getdetails(int index) async {
    await datad.workDetails(mainworks[index]['id'].toString());
    setState(() {
                  ...
                  children: [
                    ...
                    Flexible(
                      child: Text(
                        ...
                        maxLines: 3, // Maksimum 2 satır göster
                      ),
                    ),
                  ],
                ),
                    ...
                    child: Row(
                      ...
                      children: [
                        Column(
                          children: [
                            Checkbox(
                                value: (mainworks[index]['onay'] == 1
                                    ? true
                                    : false),
                                onChanged: (bool? value) => {
                                      setState(() {
                                        conf(index, value, person);
                                        //vericek();
                                      }),
                                    })
                          ],
